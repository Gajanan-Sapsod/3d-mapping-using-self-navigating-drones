    ...
    def sort_neighbours(self):
        ...
        for i in range(len(self.neighbours)):
            for j in range(len(self.neighbours)):
                if j!=len(self.neighbours)-1:
                    if(self.neighbours[j].distance>self.neighbours[j+1].distance):
                        swap=self.neighbours[j]
                        self.neighbours[j]=self.neighbours[j+1]
                        self.neighbours[j+1]=swap
...
def main():
    ...
    while True:
        ...
        #for i in range(columns):
            #for j in range(rows):
               # grid[i][j].sort_neighbours()
                
        if begin_search:
            if len(queue) > 0 and searching:
                current_box = queue.pop(0)
                current_box.visited = True
                current_box.sort_neighbours()
